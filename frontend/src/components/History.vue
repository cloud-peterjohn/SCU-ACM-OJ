...
<script>
    ...
    export default {
        ...
        mounted() {
            axios.post('/api/JudgeRecord/MyAllList', new URLSearchParams({
                username:localStorage.getItem('username')
            }))
            .then(response => {
                this.data = response.data.data.map(rec=>({
                    record_id: rec.id,
                        problem_title:rec.problemTitle,
                        user_name: rec.username,
                        template_name: rec.templateName,
                        judge_time: rec.judgeTime,
                        judge_result: rec.judgeResult,
                        time_usage: rec.runTime,
                        memory_usage: rec.runMemory,
                }));
            })
            .catch(error => {
                console.log(error);
