        ...
        <Layout>
            ...
            <Layout>
                <Layout :style="{padding: '0 24px 24px'}">
                        ...
                        <Table :columns="columns" :data="problemList">
                            ...
                            <template #result="{ row }">
                                <span>
                                    ...
                                    <Tag :color="JUDGE_RESULT[row.result]?.color">
                                        {{ row.result }}
                                    </Tag>
                                </span>
                            </template>
                    ...
                    <br>
                    <Card>
                        <template #title><h2 style="color: #8c0776;">Contest Leaderboard</h2></template>
                        <Button type="primary" @click="on_select_leader_board" style="margin-left:20px;">Enter Leaderboard</Button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            on_select_leader_board() {
                console.log("on_select_leader_board:",this.contestID);
                this.$router.push({
                    name:'user_board',
            ...
            handleView(row) {
                ...
                const contestStartTime = new Date(this.contestStartTime); 
                const contestEndTime = new Date(this.contestEndTime);
            ...
            on_select_show_contest() {
                ...
                axios.post('/api/JudgeRecord/MyList', 
                    new URLSearchParams({
                        contestId: this.contestID 
                    })
                ) 
                .then(response => { 
                    this.problemList = response.data.data.map(prob=>({
                        id: prob.problemId,
                        title: prob.problemTitle,
                        result: prob.judgeResult,
                        timeCost: prob.runTime,
                        memoryCost: prob.runMemory,
                    })); 
                }) 
                .catch(error => { 
                    console.log(error);  
